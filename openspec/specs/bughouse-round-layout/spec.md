...
thing — how a stack is built, how a board and its pocket are sized, where a strip goes, how the
merged column is arranged, how portrait is pinned — they SHALL do so through ONE set of rules, not
two that happen to agree. A rule that applies to both MUST NOT be written twice.

Only three differences are permitted, and each is a property of what the analysis page IS rather
than a divergence in layout:

1. **Eval gauges.** The analysis page draws a gauge beside each board; the round page has none. Its
   board tracks therefore carry the gauge's width where the round page's do not.
2. **No clock in the strip.** The analysis page's clocks are absolutely positioned overlays on the
   board, so its seat strip holds a pocket and a username and nothing else, and is one square tall.
   The round page's strip additionally holds a live clock and sizes it.
3. **A different tab list.** The round page's parts are placed individually so that a shrunken board
   frees space for them; the analysis page has one tools block holding its own tab list.

Anything else that differs is a defect in one of the two pages, not a decision.

#### Scenario: A layout rule applies to both pages
- **WHEN** a rule describes something both pages have
- **THEN** it is written once and scoped to both, rather than duplicated per page

#### Scenario: A difference is found that is not one of the three
- **WHEN** the two pages lay something out differently for a reason not on the list above
- **THEN** it is treated as a defect rather than as an intended difference
...
### Requirement: Portrait SHALL be pinned to the viewport, on both pages

Both two-board pages in portrait SHALL fit the viewport exactly and SHALL NOT scroll in either axis.
The cap SHALL be applied at every level between the body and the app, because a bare `1fr` is
`minmax(auto, 1fr)` whose minimum is min-content, so the lowest uncapped track wins and carries the
whole chain past the screen.

The partner's board and the tools SHALL be ONE block occupying the region above, and that block
SHALL absorb the slack, so that the viewer's own stack sits flush on the bottom edge whatever the
two boards work out to. The tools SHALL occupy the space BESIDE the small partner board rather than
a full-width row of their own between the two boards.

The viewer's own stack SHALL be exactly its ten squares — a strip, the board, a strip — and every
username SHALL stay inside its pocket strip. A name that left its strip would add a line at each end
and push the stack past the height it was given, which is the one thing a pinned layout cannot
absorb.

#### Scenario: Portrait at any width
- **WHEN** either page is displayed in portrait
- **THEN** the document scrolls in neither axis
- **AND** the viewer's own board sits flush with the bottom of the viewport

#### Scenario: The tools are placed
- **WHEN** the partner's board is a fifth of the viewport and square
- **THEN** the tools occupy the width beside it, not a row below it

#### Scenario: A username in portrait
- **WHEN** either page is displayed in portrait
- **THEN** every username is drawn inside its pocket strip
- **AND** the viewer's own stack measures exactly ten of its own squares
