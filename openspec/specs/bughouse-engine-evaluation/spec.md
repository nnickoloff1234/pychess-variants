# bughouse-engine-evaluation

## Purpose
Local engine evaluation on the two-board bughouse analysis page: one switch for both boards, one
Fairy-Stockfish instance alternating between them a ply at a time, and what the panel shows for each
board — its evaluation, the depth that evaluation was reached at, and its principal variations.
